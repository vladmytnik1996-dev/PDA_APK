...
Мобильное Android-приложение в стиле КПК из S.T.A.L.K.E.R. Оно работает автономно: карта, задания, заметки, GPS-привязка и QR-обмен хранятся на телефоне. Интернет для обычной работы не требуется.

Готовый APK находится здесь:

```text
apk/stalker-pda-v1.0.5-release.apk
```
...
| **QR** | Открывает экран получения метки или заметки через камеру либо изображение QR-кода. |
...
- **Данные** — GPS-привязка, QR-получение, резервная копия и сброс.
...
Передавать можно отдельную метку или заметку. В QR не включается сам файл карты.

Для метки или заметки нажмите **«Передать»**.
...
Передаются только данные выбранного объекта: название, описание, тип, статус, координаты и название карты.

---

## 11. Получение через камеру или изображение

Нажмите кнопку **QR** сверху или откройте **«Данные» → «Получить метку или заметку»**.
...
До подтверждения приложение показывает тип, название, описание, категорию, координаты, статус и карту источника.
...
Импорт никогда не выполняется автоматически. Если название карты отправителя отличается от вашей карты, приложение показывает предупреждение. Проверьте координаты перед добавлением.
...
8. Передавайте отдельные объекты через QR.
...
- QR-код содержит только выбранную метку или заметку;
