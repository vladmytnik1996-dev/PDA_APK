...
with sync_playwright() as p:
    ...
    assert page.locator('#mapHint').evaluate("e => getComputedStyle(e).display") != 'none'
    assert 'РЕЖИМ ПОСТАНОВКИ' in page.locator('#mapHint').inner_text().upper()
    page.locator('#mapViewport').click(position={'x':vp['width']*.55,'y':vp['height']*.45}); page.wait_for_selector('#markerModal.visible')
